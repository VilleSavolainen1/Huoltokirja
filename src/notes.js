...
const Notes = ({notesToRender, addnote, setAddnote}) => {
    ...
    const handleSubmit = (e) => {
        ...
        axios.post('/api/notes', changed)
        .then(notes => console.log('updated'))
        setAddnote(false)
    }
    

    const list = notesToRender.map(notes => {return <ul key={notes.id}><p>{notes.note} <Deletebutton target={notes.id}/></p></ul>})
    return (
        <div style={noteStyle}>
            <div>
                <Button variant="outline-light" onClick={showInputs}><img src={plus} style={imageStyle} alt="img"></img></Button><br></br><br></br>
                    { addnote ? 
                        ...
                        <form onSubmit={handleSubmit}>
                            <textarea type="text" placeholder="Uusi muistiinpano..." onChange={handleInput} rows="4" cols="50"></textarea>
